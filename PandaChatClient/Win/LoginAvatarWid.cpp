...
QPixmap LoginAvatarWid::GetSetAvatarPic()
{
	return avatar_pix;
}
...
void LoginAvatarWid::SetUserCurState(USER_STATE curstate)
{
	...
	{
	case State_Online:
		btn_login_state->setStyleSheet("border-image: url(:/PandaChatClient/all_image/Win_Login/state_online.png);");
		break;
	case State_Leave:
		btn_login_state->setStyleSheet("border-image: url(:/PandaChatClient/all_image/Win_Login/state_leave.png);");
		break;
	case State_Busy:
		btn_login_state->setStyleSheet("border-image: url(:/PandaChatClient/all_image/Win_Login/state_busy.png);");
		break;
	case State_NotDisturb:
		btn_login_state->setStyleSheet("border-image: url(:/PandaChatClient/all_image/Win_Login/state_ban.png);");
		break;
	case State_InVisable:
		btn_login_state->setStyleSheet("border-image: url(:/PandaChatClient/all_image/Win_Login/state_yinshen.png);");
		break;
	default:
		break;
	}
...
void LoginAvatarWid::SetStateBtnEnable(bool flag)
{
		ui->btn_login_state->setEnabled(flag);
}
