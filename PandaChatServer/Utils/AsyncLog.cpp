...
bool CAsyncLog::output(long nLevel, const char* pszFileName, int nLineNo, const char* pszFmt, ...)
{
    ...
    std::string strLine;
    makeLinePrefix(nLevel, strLine);  //拼接日志等级+时间+线程信息
    ...
    //容量必须算上最后一个\0
    if ((int)strLogMsg.capacity() < nLogMsgLength + 1)//capacity 返回string目前分配的最大存储容量
    {
        strLogMsg.resize(nLogMsgLength + 1);
    }
...
void CAsyncLog::writeThreadProc()
{
    ...
    while (true)
    {       
        if (!m_strFileName.empty()) //没有保存的文件名
        {
			//第一次操作没有文件 或者文件已写入字节数超过最大限制 新建文件
		...
		std::string strLine;
        {
            ...
            strLine = m_listLinesToWrite.front(); //返回list第一个元素引用
			m_listLinesToWrite.pop_front();//删除list删除链表头的一元素
        }
#ifdef _WIN64
        //OutputDebugStringA(strLine.c_str());
        //OutputDebugStringA("\n");
