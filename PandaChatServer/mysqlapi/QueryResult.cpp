 ...
 : m_FieldCount(fieldCount), m_RowCount(rowCount)
{
	...
	//mysql_fetch_fields返回结果集中代表字段（列）的对象的数组，返回结果集中代表字段（列）的对象的数组，然后输出每个字段名称、表格和最大长度
...
bool QueryResult::nextRow()
{
	...
	//mysql_fetch_row检索一个结果集合的下一行。当在mysql_store_result()之后使用时，如果没有更多的行可检索时，mysql_fetch_row()返回NULL。
	...
	//mysql_fetch_lengths返回结果集的当前行的每一列的数据长度。尤其是当结果包含二进制数据，必须使用mysql_fetch_lengths()来获得数据的长度，只对当前行有效
